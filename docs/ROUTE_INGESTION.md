...
Ascent Ledger imports named routes only from open data and source APIs whose reuse terms permit redistribution. It never scrapes proprietary guidebooks or route sites. `Route` is the user-facing canonical object; every imported representation is retained in `RouteSourceRecord` with its original ID, URL, metadata, geometry, licence, attribution, timestamps, checkpoint and active/stale lifecycle.
...
1. collect only relevant route relations/direct climbing or ferrata ways;
...
Field precedence is official agency (400) > OSM (300) > open community catalogue (200) > inference (100). Each canonical field records its winning source and precedence. Editing a route in the UI marks those fields `userEdited` at precedence 1000, so imports cannot overwrite them blindly.
